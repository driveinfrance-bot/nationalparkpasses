...
export async function POST(request: Request) {
  ...
  const responseId = data.responseId ?? data.submissionId ?? payload.responseId ?? payload.submissionId;
  ...
  const vehicleRegistration = getStringValue(getFieldValue(["Vehicle registration"]));
  const country = getStringValue(getFieldValue(["Country"]));
  const vehicleType = getStringValue(getFieldValue(["Vehicle type"]));
  const fuelType = getStringValue(getFieldValue(["Fuel type"]));
  const firstRegistrationDateRaw = getStringValue(
    getFieldValue(["First registration date"])
  );
  const addressName = getStringValue(getFieldValue(["Address name"]));
  const addressStreet = getStringValue(getFieldValue(["Street"]));
  ...
  const addressPostalCode = getStringValue(getFieldValue(["Postal code"]));
  const addressCountry = getStringValue(getFieldValue(["Address country"]));
  ...
  const order = await prisma.order.create({
    data: {
      tallyResponseId: responseId ?? null,
      email: email || "",
      name: name || undefined,
      ...
      fuelType: fuelType || "",
      firstRegistrationDate: firstRegistrationDateRaw
